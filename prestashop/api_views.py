...
"""
            ...
            if obj['what']=='new':
                ...
                for p in queryset:
                    ...
                    # 'New' attribute is used incorrectly in multi-shop - some views use ps17_product and some ps17_product_shop so we set both

                    if set_new=='n':
                        sql = "update ps17_product_shop set date_add=DATE_SUB(NOW(),INTERVAL 1 DAY) where id_product=%s and id_shop=%s"
                        sql1 = "update ps17_product set date_add=DATE_SUB(NOW(),INTERVAL 1 DAY) where id_product=%s"
                        ...
                        sql1 = "update ps17_product set date_add=DATE_SUB(NOW(),INTERVAL 1 YEAR) where id_product=%s"
                        sql2 = "delete from ps17_feature_product where id_product=%s and id_feature=%s"
                        sql3 = ''    
                    elif set_new=='b':  
                        sql = "update ps17_product_shop set date_add=DATE_SUB(NOW(),INTERVAL 2 DAY) where id_product=%s and id_shop=%s"
                        sql1 = "update ps17_product set date_add=DATE_SUB(NOW(),INTERVAL 2 DAY) where id_product=%s"
                        sql2 = ''    
                    ...
                    with connections[db].cursor() as cursor:
                        if sql:
                            ...
                            n_updated += 1
                        if sql1:
                            cursor.execute(sql1,[p.id_product])
                        if sql2:

...
class Product(models.Model):
    ...
    def barcode_tag(self):
        return mark_safe(
            f"<svg id='barcode_{self.id_product}'></svg>"
            + "<script>"
...
class Order(models.Model):
    ...
    subtotal = models.DecimalField(
        "subtotal", max_digits=8, decimal_places=2, default=0
    )
    ...
    discount_pc = models.IntegerField(
        "discount_pc", default=DiscountPc.PC20, choices=DiscountPc.choices
    )
    discount_gbp = models.DecimalField(
        "discount_gbp", default=0, max_digits=8, decimal_places=2
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self._state.adding:
            if self.status == Order.Status.PAID and (
                self._state.adding or self._loaded_values["status"] == Order.Status.NEW
            ):
                # do something
                need_recalc = True
...
class OrderDetail(models.Model):
    order = models.ForeignKey(
        Order, on_delete=models.CASCADE, related_name="orderdetail_set"
    )
